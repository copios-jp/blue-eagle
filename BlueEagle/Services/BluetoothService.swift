...
class BluetoothService: NSObject {
  ...
  init(_ eventBus: EventBus = NotificationCenter.default, manager: BluetoothHeartRateMonitorService = CBCentralManager()) {
  ...
  func disconnectAll() {
    let connected: [CBPeripheral] = manager.retrieveConnectedPeripherals(withServices: [GATT.heartRateMonitor])
...
extension BluetoothService: CBPeripheralDelegate {
  ...
  func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error _: Error?) {
    ...
    guard let heartRateMeasurement = characteristics.first(where: { $0.uuid == GATT.heartRateMeasurement }) else { return }
