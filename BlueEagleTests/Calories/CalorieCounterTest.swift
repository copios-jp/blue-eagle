...
final class CalorieCounterTest: XCTestCase {
  ...
  func configure(_ sex: Sex, _ age: Int, _ weight: Int) {
      User.current.sex = sex
      User.current.birthdate = Date()
      User.current.weight = weight
  }

  override func setUpWithError() throws {
      subject = CalorieCounter(user: User.current)
    training = Training()
  ...
  func testCaloriesOnePerMinuteFemaleSample() throws {
    ...
    configure(Sex.female, age, weight)
  ...
  func testCaloriesAroundMinimumSampleRatePerMinute() throws {
    ...
    configure(Sex.undeclared, age, weight)
